class EducationCredits extends HTMLElement {
    async connectedCallback() {

        let user = await API_getUserByWebId(window.webId);
        let percentage = (user.educationCredits / 1000) * 100;

        this.innerHTML = `
<style>
.grid-3-col > education-credits {
    padding: 20px;
...
<div id="${user.pubKey}-hc">
        ...
        <ul class="funds-raised">
            ...
            <li>
                Current Balance<b class="pull-right"><span class="creditsReceived">${user.educationCredits}</span> ${user.defaultCurrency}</b>
            </li>
        </ul>
...
customElements.define('education-credits', EducationCredits);
